...
class API:
    _API_AUTH = 'https://api.authentication.husqvarnagroup.dev/v1/'
    _API_TRACK = 'https://api.amc.husqvarna.dev/v1/'
    ...
    def login(self, login, password):
        response = self.session.post(self._API_AUTH + 'oauth2/token',
                                     headers=self._HEADERS,
                                     data='grant_type=client_credentials&client_id=%s&client_secret=%s&code=&redirect_uri=&refresh_token=' %(login, password)
                                     )
        print(response.text)
        ...
        json = response.json()
        self.set_token(login, json["access_token"], json["provider"])
        return json["expires_in"]

    def logout(self, login):
        response = self.session.delete(self._API_AUTH + 'token/%s' % self.token)
        header = {'Accept': '*/*', 'x-api-key': login}
        response.raise_for_status()
        self.device_id = None
        self.token = None
    ...
    def set_token(self, login, token, provider):
        self.token = token
    ...
    def list_robots(self):
        response = self.session.get(self._API_TRACK + 'mowers', headers=self.session.headers)
        response.raise_for_status()
    ...
    def status(self):
        response = self.session.get(self._API_TRACK + 'mowers/%s' % self.device_id, headers=self.session.headers)
        response.raise_for_status()
    ...
    def control(self, command):
        if command not in ['PARK', 'STOP', 'START', 'PAUSE', 'RESUME']:
        ...
        response = self.session.post(self._API_TRACK + 'mowers/%s/actions' % self.device_id,
    ...
    def command_force_start(self, duration):
...
def setup_api(config, tokenConfig, args):
    ...
    if args.token and tokenConfig.token_valid():
        mow.set_token(config.login, tokenConfig.token, tokenConfig.provider)
    else:
    ...
    def do_GET(self):
        ...
        while retry > 0:
            try:
                ...
                if self.path == '/start':
                    mow.control('RESUME')		# Not possible to force start for now
                    self.send_response(200)
                    self.end_headers()
...
parser.add_argument('--login', dest='login', help='Your AppKey')
